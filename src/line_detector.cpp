...
bool LineDetector::merge_segments(SEGMENT *seg1, SEGMENT *seg2,
  ...
  double middist = sqrt((seg1mid.x - seg2mid.x) * (seg1mid.x - seg2mid.x) +
...
void LineDetector::extract_segments(vector<Point2i> *points,
  ...
  for (i = 0; i + threshold_length_ < total; i++) {
    ...
    if (is_line == false) continue;

    l_points.push_back(pe);

    Vec4f line;
#if (CV_VERSION_MAJOR >= 4)
    fitLine(Mat(l_points), line, cv::DIST_L2, 0, 0.01, 0.01);
#else
...
bool LineDetector::get_point_chain(const Mat &img, const Point &pt,
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (ri < 0 || ri == img.rows || ci < 0 || ci == img.cols) continue;

    if (img.at<unsigned char>(ri, ci) == 0) continue;
...
void LineDetector::get_angle(SEGMENT *seg) {
  ...
  if (ang > 2.0 * CV_PI) ang -= 2.0 * CV_PI;
  seg->angle = (float)ang;
}
...
void LineDetector::additional_operations_on_segments(Mat &src, SEGMENT *seg) {
  ...
  if (iR > iL) {
    ...
    ang = ang + CV_PI;
    if (ang >= 2.0 * CV_PI) ang = ang - 2.0 * CV_PI;
    seg->angle = (float)ang;
  }
